...
export default function UnifiedDigestViewer({
    ...
    const handleDeleteImage = async () => {
        ...
        try {
            const apiUrl = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000';
            const res = await fetch(`${apiUrl}/digests/${digestData.id}/image`, {
                method: 'DELETE',
                headers: {
                    'Authorization': `Bearer ${localStorage.getItem('token')}`
                }
            ...
            if (res.ok) {
                // Update local state by forcing a re-render or clearing local prop copy if available
                // Since digestData is a prop, we need to notify parent or hack it visually.
                // We can use the same pattern as Generate Image: setDigestSummary logic if it updated tokens, 
                // but here we just removed the 'image_url' field.
                // The parent (NewsGlobe) likely needs to refetch.
                // However, we can simply hide it visually for now.
                if (isEditing && setDigestSummary) {
                    // Try to remove token or markdown link if present
                    let text = digestDataRef.current?.digest || "";
                    // Remove ![...](...) links that point to our image?
                    // Actually, the main image is rendered explicitly via digestData.image_url.
                    // We just need to hide <img />
                }

                // Force visual update by mutating the ref (dangerous but works) or alerting parent?
                // UnifiedDigestViewer doesn't have onUpdate callback.
                // We'll optimistically update the UI by setting imageError to hide it, 
                // OR we rely on parent re-fetch if we had a callback.
                // Let's assume user will refresh or we can reload.
                // Better: Force reload page or call onSave?
                alert("Image removed. It will disappear on next refresh.");
                setImageError(true); // Hides the image
            } else {
                            ...
                            <div className="mb-12 flex justify-center relative group">
                                ...
                                {!isReadOnly && !imageError && (
                                    <button
                                        ...
                                        className="absolute top-2 right-2 p-1.5 bg-black/60 hover:bg-red-600/90 text-white rounded-full transition-all duration-200 backdrop-blur-sm z-20"
                                        title="Remove Image"
                                    >
                                        <X className="w-4 h-4" />
                                    </button>
